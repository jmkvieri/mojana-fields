...
threshold <- quantile(nodes_df$betweenness, 0.90)
nodes_df_filtered <- nodes_df %>% filter(betweenness > threshold)
...
# Visualize only the most central nodes on the plot (BETWEENNESS CENTRALITY)
e <- ggplot() +
  geom_sf(data = camellones, color = "grey") +
  geom_sf(data = nodes_sf_filtered, aes(size = betweenness), color = "red") +
  theme_minimal() +
  labs(
    title = "Nodes with high betweenness centrality",
    size = "Betweenness\nCentrality") +
